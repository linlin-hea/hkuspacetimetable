...
function renderTimetable() {
  ...
  const sessions = selected.flatMap(key => classSessions(key).map(item => ({...item, isAlternative: false})));
...
$('confirmReplace').addEventListener('click', () => { const original=$('originalSelect').value; const replacements=[...$('replacementSelect').selectedOptions].map(option => option.value); if (!replacements.length) { toast('請至少選擇一個替換班別。', true); return; } const error=replacements.map(replacement => validateReplacement(original,replacement)).find(Boolean); if (error) { closeModal(); toast(error, true); return; } alternativeOriginalKey=original; alternativeKeys=replacements; alternativeKey=replacements.length === 1 ? replacements[0] : null; closeModal(); renderAll(); toast(`已顯示 ${replacements.length} 個替換班別預覽。`); });
