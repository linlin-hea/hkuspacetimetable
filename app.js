...
function loadUserState(name, migrateLegacy = false) { try { const stored = localStorage.getItem(userStorageKey(name)); if (!stored && migrateLegacy && !localStorage.getItem('spacePlannerLegacyMigrated')) { const legacyPlans = validPlans(JSON.parse(localStorage.getItem('spacePlannerPlans') || '[]')); localStorage.setItem('spacePlannerLegacyMigrated', '1'); if (legacyPlans.length) return {selected: [], savedPlans: legacyPlans, originalSelected: [], replacementHistory: []}; } const value = JSON.parse(stored || '{}'); return {selected: Array.isArray(value.selected) ? value.selected.filter(key => typeof key === 'string') : [], savedPlans: validPlans(value.savedPlans), originalSelected: Array.isArray(value.originalSelected) ? value.originalSelected.filter(key => typeof key === 'string') : [], replacementHistory: validReplacementHistory(value.replacementHistory)}; } catch { return {selected: [], savedPlans: [], originalSelected: [], replacementHistory: []}; } }
function syncUserState() { if (!currentUser || !remoteHydrated) return; fetch('/api/profile', {method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify({selected, savedPlans, originalSelected, replacementHistory})}).then(response => { if (response.status === 401) throw new Error('session expired'); if (!response.ok) throw new Error('sync failed'); }).catch(() => toast('伺服器同步失敗，已保留本機資料。', true)); }
function persistUserState() { if (!currentUser) return true; try { localStorage.setItem(userStorageKey(currentUser), JSON.stringify({selected, savedPlans, originalSelected, replacementHistory})); localStorage.setItem('spacePlannerCurrentUser', currentUser); syncUserState(); return true; } catch { toast('個人資料未能儲存，請檢查瀏覽器儲存空間。', true); return false; } }
async function fetchUserState(name) { try { const response=await fetch('/api/profile'); if (response.status === 401) return null; if (!response.ok) throw new Error('profile unavailable'); const value=await response.json(); return {selected:Array.isArray(value.selected) ? value.selected.filter(key => typeof key === 'string') : [], savedPlans:validPlans(value.savedPlans), originalSelected:Array.isArray(value.originalSelected) ? value.originalSelected.filter(key => typeof key === 'string') : [], replacementHistory:validReplacementHistory(value.replacementHistory), isAdmin:Boolean(value.isAdmin)}; } catch { return loadUserState(name); } }
...
async function logoutUser() { persistUserState(); await fetch('/api/auth/logout', {method:'POST'}).catch(() => {}); currentUser=''; isAdmin=false; selected=[]; savedPlans=[]; originalSelected=[]; replacementHistory=[]; alternativeOriginalKey=null; alternativeKey=null; alternativeKeys=[]; remoteHydrated=false; localStorage.removeItem('spacePlannerCurrentUser'); updateUserBar(); renderAll(); openLogin(); }
...
function validateAddKey(key, batch = []) { if (!classSessions(key).length) return '找不到這個 Course Code / Class No 組合。'; if (selected.includes(key) || pendingCourseKeys.includes(key) || batch.includes(key)) return '這個課堂已經在加入清單中。'; if (hasScheduleConflict([...selected, ...pendingCourseKeys, ...batch, key])) return '課堂時間衝突，無法加入課堂。'; return null; }
...
function arrayBufferToBase64(buffer) { let binary=''; const bytes=new Uint8Array(buffer); const chunkSize=0x8000; for (let i=0;i<bytes.length;i+=chunkSize) binary+=String.fromCharCode(...bytes.subarray(i,i+chunkSize)); return btoa(binary); }
 ...
 $('confirmAddCourse').addEventListener('click', () => { const {code, key}=currentAddKey(); const batch=[...pendingCourseKeys]; if ($('courseCodeInput').value.trim() || $('classNoInput').value.trim()) { const error=validateAddKey(key, batch); if (error) { toast(error, true); return; } batch.push(key); } if (!batch.length) { toast('請至少輸入一個課堂。', true); return; } if (hasScheduleConflict([...selected, ...batch])) { toast('課堂時間衝突，無法加入課堂。', true); return; } selected=[...selected, ...batch]; closeAddModal(); renderAll(); toast(batch.length === 1 ? `已加入 ${displayClass(batch[0])}。` : `已加入 ${batch.length} 門課堂。`); });
...
if (currentUser) { const state=loadUserState(currentUser); selected=state.selected; savedPlans=state.savedPlans; originalSelected=state.originalSelected; replacementHistory=state.replacementHistory; }
updateUserBar();
renderAll();
if (!currentUser) openLogin();
else fetchUserState(currentUser).then(state => { if (!state) { currentUser=''; isAdmin=false; selected=[]; savedPlans=[]; originalSelected=[]; replacementHistory=[]; localStorage.removeItem('spacePlannerCurrentUser'); updateUserBar(); renderAll(); openLogin(); return; } selected=state.selected; savedPlans=state.savedPlans; originalSelected=state.originalSelected; replacementHistory=state.replacementHistory; isAdmin=state.isAdmin; remoteHydrated=true; updateUserBar(); renderAll(); });
