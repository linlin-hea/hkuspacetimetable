...
const requestHandler = async (req, res) => {
  try {
    ...
    if (req.method === 'GET') return sendJson(res, 200, {name: profiles[name]?.name || displayName, isAdmin: isAdminUser(name), ...(profiles[name] || {selected: [], savedPlans: [], originalSelected: [], replacementHistory: [], replacementKeys: [], courseMeta: {}})});
      const payload = JSON.parse(await readRequestBody(req, 2 * 1024 * 1024)); const courseMeta = payload.courseMeta && typeof payload.courseMeta === 'object' && !Array.isArray(payload.courseMeta) ? Object.fromEntries(Object.entries(payload.courseMeta).filter(([, item]) => item && typeof item === 'object').map(([key, item]) => [key, {instructor:typeof item.instructor === 'string' ? item.instructor.slice(0, 120) : '', note:typeof item.note === 'string' ? item.note.slice(0, 2000) : ''}])) : {}; profiles[name] = {name: profiles[name]?.name || displayName, selected: Array.isArray(payload.selected) ? payload.selected.filter(item => typeof item === 'string') : [], savedPlans: Array.isArray(payload.savedPlans) ? payload.savedPlans : [], originalSelected: Array.isArray(payload.originalSelected) ? payload.originalSelected.filter(item => typeof item === 'string') : [], replacementHistory: Array.isArray(payload.replacementHistory) ? payload.replacementHistory.filter(item => item && typeof item.from === 'string' && typeof item.to === 'string').slice(-100) : [], replacementKeys: Array.isArray(payload.replacementKeys) ? payload.replacementKeys.filter(item => typeof item === 'string') : [], courseMeta}; writeProfiles(profiles); return sendJson(res, 200, {ok:true});
    }
