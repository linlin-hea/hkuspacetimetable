...
const requestHandler = async (req, res) => {
  try {
    ...
    if (req.method === 'GET') return sendJson(res, 200, {name: profiles[name]?.name || displayName, isAdmin: isAdminUser(name), ...(profiles[name] || {selected: [], savedPlans: [], originalSelected: [], replacementHistory: [], replacementKeys: []})});
      const payload = JSON.parse(await readRequestBody(req, 2 * 1024 * 1024)); profiles[name] = {name: profiles[name]?.name || displayName, selected: Array.isArray(payload.selected) ? payload.selected.filter(item => typeof item === 'string') : [], savedPlans: Array.isArray(payload.savedPlans) ? payload.savedPlans : [], originalSelected: Array.isArray(payload.originalSelected) ? payload.originalSelected.filter(item => typeof item === 'string') : [], replacementHistory: Array.isArray(payload.replacementHistory) ? payload.replacementHistory.filter(item => item && typeof item.from === 'string' && typeof item.to === 'string').slice(-100) : [], replacementKeys: Array.isArray(payload.replacementKeys) ? payload.replacementKeys.filter(item => typeof item === 'string') : []}; writeProfiles(profiles); return sendJson(res, 200, {ok:true});
    }
